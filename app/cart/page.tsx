...
function CartItemImage({ src, alt }: { src: string; alt: string }) {
  const [imageError, setImageError] = useState(false)
  const placeholderImage = '/images/placeholder.png'
  const imageSrc = imageError || !src ? placeholderImage : src
...
export default function CartPage() {
  ...
  const [promoApplied, setPromoApplied] = useState(false)
  const [promoError, setPromoError] = useState('')
  const [savingForLater, setSavingForLater] = useState<string[]>([]) // Changed to string[]

  // Wrapper functions to handle string IDs
  ...
  // Group items by category or supplier (example using a map function)
  const groupedItems = items.reduce((groups, item) => {
    // Use a more deterministic way to group items (using SKU or product name prefix)
    // This is just an example - you should use actual category from product data
    let category = 'Other Products'
    
    // Try to determine category from product name or SKU
    if (item.name.toLowerCase().includes('equipment') || item.name.toLowerCase().includes('apparatus')) {
      category = 'Laboratory Equipment'
    } else if (item.name.toLowerCase().includes('chemical') || item.name.toLowerCase().includes('reagent')) {
      category = 'Chemicals & Reagents'
    } else if (item.name.toLowerCase().includes('glass') || item.name.toLowerCase().includes('beaker') || item.name.toLowerCase().includes('flask')) {
      category = 'Glassware'
    } else if (item.name.toLowerCase().includes('safety') || item.name.toLowerCase().includes('glove') || item.name.toLowerCase().includes('goggle')) {
      category = 'Safety Gear'
    } else if (item.name.toLowerCase().includes('consumable') || item.name.toLowerCase().includes('tip') || item.name.toLowerCase().includes('tube')) {
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              {Object.entries(groupedItems).map(([category, categoryItems]) => (
                <div key={category} className="bg-[var(--white)] dark:bg-[var(--brand-800)] rounded-2xl shadow-lg border border-[var(--card-border)] overflow-hidden">
                  ...
                  <div className="divide-y divide-[var(--card-border)]">
                    {categoryItems.map((item) => (
                      <div key={String(item.id)} className="relative p-6 group hover:bg-[var(--soft-gray)] dark:hover:bg-[var(--brand-800)]/50 transition-all duration-300">
                        <div className="flex gap-6">
                          {/* Product Image */}
                          <CartItemImage src={item.image} alt={item.name} />
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-3">
                                <button
                                  onClick={() => handleUpdateQuantity(String(item.id), item.quantity - 1)}
                                  className="p-2 bg-[var(--soft-gray)] dark:bg-[var(--brand-700)] rounded-lg hover:bg-[var(--brand-100)] dark:hover:bg-[var(--brand-900)] transition-all duration-300 disabled:opacity-50"
                                  disabled={item.quantity <= 1}
                              ...
                              <div className="flex items-center gap-3">
                                <button
                                  onClick={() => handleSaveForLater(String(item.id))}
                                  className="text-sm text-[var(--card-border)] hover:text-[var(--brand-600)] dark:text-[var(--card-border)] dark:hover:text-[var(--brand-400)] transition-colors duration-300"
                                >
                                  Save for later
                                </button>
