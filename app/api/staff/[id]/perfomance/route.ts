...
// app/api/staff/[id]/performance/route.ts
/*import { NextRequest, NextResponse } from 'next/server';
import { Staff } from '@/models/Staff';
import { Types } from 'mongoose';
...
export async function GET(request: NextRequest, { params }: StaffRouteContext) {
  try {
    ...
    const currentMonthActivity = (staff.activity || []).filter(
      (a: any) => new Date(a.timestamp) >= currentMonthStart
    );
    const lastMonthActivity = (staff.activity || []).filter(
      (a: any) => new Date(a.timestamp) >= lastMonthStart && new Date(a.timestamp) < currentMonthStart
    );
    ...
    const insights = [];
    if (metrics.errorRatePercent > 5) {
      insights.push({ type: 'warning', message: 'Error rate is above team average.', metric: 'errorRate' });
    }
    if (metrics.ordersCreatedThisMonth > 20) {
      insights.push({ type: 'success', message: 'Outstanding order volume this month!', metric: 'volume' });
    }
    if (comparison.ordersCreated.current < comparison.ordersCreated.previous * 0.8) {
      insights.push({ type: 'info', message: 'Order creation is down from last month.', metric: 'trend' });
    }
