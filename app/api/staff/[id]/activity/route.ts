...
// app/api/staff/[id]/activity/route.ts
/*import { NextRequest, NextResponse } from 'next/server';
import { Staff } from '@/models/Staff';
import { Types } from 'mongoose';
...
export async function GET(request: NextRequest, { params }: StaffRouteContext) {
  try {
    ...
    const staff = await Staff.findById(id)
      .select('activity firstName lastName')
      .lean();
    ...
    const summary = {
      ...
      today: activities.filter(
        (a: any) => new Date(a.timestamp).setHours(0, 0, 0, 0) === today.getTime()
      ).length,
      thisWeek: activities.filter(
        (a: any) => new Date(a.timestamp) >= new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)
      ).length
    };
...
// Helper to format action strings nicely
function formatActionDisplay(action: string): string {
  return action
    .split('_')
