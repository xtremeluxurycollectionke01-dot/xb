...
// app/api/staff/[id]/availability/route.ts
import { NextRequest, NextResponse } from 'next/server';
import { Staff } from '@/models/Staff';
import { Types } from 'mongoose';
...
export async function GET(request: NextRequest, { params }: StaffRouteContext) {
  try {
    ...
    const timeStr = `${String(checkTime.getHours()).padStart(2, '0')}:${String(
    ...
    if (!daySchedule?.isWorkingDay) {
      ...
      timeStr >= daySchedule.breakStart &&
      timeStr <= daySchedule.breakEnd
    ) {
      status = 'on-break';
      reason = `On break until ${daySchedule.breakEnd}`;
    } else {
      // Check workload from recent activity
      const oneHourAgo = new Date(checkTime.getTime() - 60 * 60 * 1000);
      const recentActivity = (staff.activity || []).filter(
        (a: any) =>
          new Date(a.timestamp) >= oneHourAgo &&
          ['ORDER_CREATED', 'ORDER_PACKED', 'INVOICE_ISSUED'].includes(a.action)
      );
    ...
    const conflictingActivities = (staff.activity || []).filter((a: any) => {
      const actTime = new Date(a.timestamp);
      return actTime >= checkTime && actTime <= checkEndTime && a.action === 'MEETING_SCHEDULED';
    });
    ...
    return NextResponse.json({
      ...
      workload: {
        recentTasks: (staff.activity || []).filter(
          (a: any) => new Date(a.timestamp) >= new Date(checkTime.getTime() - 24 * 60 * 60 * 1000)
        ).length,
        queueDepth: 0
      },
      nextAvailable: nextAvailable?.toISOString(),
