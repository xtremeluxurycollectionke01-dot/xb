...
function calculateIsOnSchedule(schedule: any): boolean {
  ...
  const today = days[now.getDay()];
  const daySchedule = schedule[today];
  if (!daySchedule?.isWorkingDay) return false;
  const currentTime = `${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}`;
  return currentTime >= daySchedule.start && currentTime <= daySchedule.end;
}

// Helper: merge role and custom permissions
function calculateEffectivePermissions(rolePerms: any[], customPerms: any[] = []) {
  const merged = [...(rolePerms || [])];
  for (const custom of customPerms) {
    const index = merged.findIndex(p => p.resource === custom.resource);
    if (index >= 0) merged[index] = custom;
...
export async function GET(request: NextRequest, { params }: StaffRouteContext) {
  try {
    ...
    if (!isOwnProfile && !isAdmin) {
      const targetStaff = await Staff.findById(id);
      if (targetStaff && targetStaff.user?.toString() === auth.userId) isOwnProfile = true;
    }
    ...
    const enrichedStaff = {
      ...
      initials: `${staff.firstName[0]}${staff.lastName[0]}`.toUpperCase(),
      isOnSchedule: calculateIsOnSchedule(staff.schedule),
      effectivePermissions: calculateEffectivePermissions(staff.role?.permissions, staff.customPermissions)
    };
...
export async function PATCH(request: NextRequest, { params }: StaffRouteContext) {
  try {
    ...
    const updateData: any = {};
    const allowedFields = ['firstName', 'lastName', 'phone', 'photo', 'timezone'];
    if (isAdmin) allowedFields.push('email', 'isActive', 'role');

    allowedFields.forEach(field => {
      if (body[field] !== undefined) updateData[field] = body[field];
    });

    if (body.role && isAdmin && roleTemplates[body.role]) {
      updateData.role = roleTemplates[body.role];
    }

    const staff = await Staff.findByIdAndUpdate(id, { $set: updateData }, { new: true, runValidators: true });
    if (!staff) return NextResponse.json({ error: 'Staff not found' }, { status: 404 });
    ...
    return NextResponse.json({
      data: { ...staff.toObject(), fullName: `${staff.firstName} ${staff.lastName}` },
      message: 'Staff updated successfully'
    });
...
export async function DELETE(request: NextRequest, { params }: StaffRouteContext) {
  try {
    ...
    if (auth.staffId) {
      currentStaff = await Staff.findById(new Types.ObjectId(auth.staffId));
      if (currentStaff) isAdmin = currentStaff.hasPermission('STAFF', 'ADMIN') || hasPermission(auth, 'STAFF', 'ADMIN');
    }
    ...
    const staffToDelete = await Staff.findById(id);
    if (staffToDelete?.role?.name === 'SYSTEM_ADMIN') {
      const adminCount = await Staff.countDocuments({ 'role.name': 'SYSTEM_ADMIN', isActive: true });
      if (adminCount <= 1)
        return NextResponse.json({ error: 'Cannot deactivate last administrator' }, { status: 400 });
    }

    const result = await Staff.findByIdAndUpdate(id, { $set: { isActive: false, leftAt: new Date() } }, { new: true });
    ...
    return NextResponse.json({
      message: 'Staff deactivated successfully',
      data: { ...result.toObject(), fullName: `${result.firstName} ${result.lastName}` }
    });
