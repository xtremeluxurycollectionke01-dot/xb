...
// Fixed CheckoutItem component to accept CartItem type
function CheckoutItem({ item }: { item: CartItem }) {
  const [imageError, setImageError] = useState(false)
  const placeholderImage = '/images/placeholder.png'
  const imageSrc = imageError || !item.image ? placeholderImage : item.image
...
export default function CheckoutPage() {
  const router = useRouter()
  const { items, total, subtotal, tax, shipping: cartShipping, clearCart } = useCart()
...
const handlePlaceOrder = async () => {
  ...
  const finalTotal = subtotal - (promoApplied ? promoDiscount : 0) + (tax || 0) + shippingCost
  const selectedAddress = addresses.find(a => a.id === selectedAddressId)
...
// app/checkout/page.tsx - Updated with M-Pesa STK Push integration
'use client'
...
export default function CheckoutPage() {
  ...
  // Format phone number for M-Pesa (remove any non-digit characters and ensure it starts with 254)
  const formatMpesaPhoneNumber = (rawPhone: string): string => {
    // Remove any non-digit characters
    let cleaned = rawPhone.replace(/\D/g, '')
    
    // Check if it starts with 0 (e.g., 0712345678)
    if (cleaned.startsWith('0')) {
      cleaned = '254' + cleaned.substring(1)
    }
    // Check if it starts with 254 already
    else if (cleaned.startsWith('254')) {
      // Keep as is
    }
    // Check if it's a 9-digit number starting with 7 (e.g., 712345678)
  ...
  /*const initiateMpesaPayment = async (orderId: string, amount: number, phoneNumber: string): Promise<{ success: boolean; transactionId?: string; error?: string }> => {
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || data.message || 'Failed to initiate M-Pesa payment')
      }
      ...
      if (data.ResponseCode === '0' || data.ResponseCode === 0) {
        return {
          success: true,
          transactionId: data.CheckoutRequestID || data.MerchantRequestID
        }
      } else {
        return {
          success: false,
...
const initiateMpesaPayment = async (orderId: string, orderNumber: string, amount: number, phoneNumber: string): Promise<{ success: boolean; paymentId?: string; checkoutRequestId?: string; error?: string }> => {
  ...
  const checkPaymentStatus = async (orderId: string, maxAttempts = 30, interval = 3000): Promise<{ success: boolean; mpesaCode?: string }> => {
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        // Check order status from your API
        ...
        if (data.success && data.data) {
          ...
          if (order.status === 'CONFIRMED' || order.status === 'PACKING' || order.status === 'PACKED') {
            // Find M-Pesa transaction in payments
            const mpesaPayment = order.payments?.find((p: any) => p.method === 'mpesa')
            return {
              success: true,
              mpesaCode: mpesaPayment?.reference
            }
          }
  ...
  const handlePlaceOrder = async () => {
    // Validation
    if (selectedPayment === 'mpesa' && !mpesaPhone) {
    ...
    try {
      ...
      const orderData = {
        destination: {
          deliveryPointId: '67b8c8a1e6b3b5a1b3c4d5e6', // Default delivery point
          name: selectedAddress ? `${selectedAddress.firstName} ${selectedAddress.lastName}` : 'Guest',
          address: selectedAddress ? 
            `${selectedAddress.address}${selectedAddress.apartment ? `, ${selectedAddress.apartment}` : ''}, ${selectedAddress.city}, ${selectedAddress.state} ${selectedAddress.zipCode}` :
            '',
        ...
        taxRate: 16,
        deposit: 0,
        payments: selectedPayment === 'mpesa' ? [] : [], // No payment recorded yet - will come from callback
        requestedDeliveryDate: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(),
        orderNotes: orderNotes,
        promoCode: promoApplied ? promoCode : undefined,
        promoDiscount: promoDiscount
      ...
      if (result.success && result.data.order) {
        ...
        // If M-Pesa payment is selected, initiate STK push
        if (selectedPayment === 'mpesa') {
          setIsMpesaProcessing(true)
          
          // Show toast notification that STK push is being sent
          ...
          if (!paymentResult.success) {
            toast.error(paymentResult.error || 'Failed to initiate M-Pesa payment', { id: 'mpesa-payment' })
            setIsProcessing(false)
          ...
          toast.success('Check your phone for the M-Pesa prompt', { id: 'mpesa-payment' })
          
          // Wait for payment confirmation
          toast.loading('Waiting for payment confirmation...', { id: 'mpesa-confirm' })
          
          const confirmation = await checkPaymentStatus(order.id)
          
          if (confirmation.success) {
            toast.success(`Payment successful! Transaction ID: ${confirmation.mpesaCode}`, { id: 'mpesa-confirm' })
            setMpesaTransactionId(confirmation.mpesaCode || '')
            setOrderNumber(order.orderNumber)
            setOrderComplete(true)
            clearCart()
          } else {
            toast.error('Payment confirmation timeout. Your order will be processed once payment is confirmed.', { id: 'mpesa-confirm' })
            // Still set order as placed but pending payment
            setOrderNumber(order.orderNumber)
            setOrderComplete(true)
            clearCart()
          }
        } else {
          // For other payment methods, just confirm the order
          const confirmResponse = await fetch(`/api/orders/${order.id}/confirm`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
        ...
        <Container className="relative z-10">
          <div className="max-w-2xl mx-auto text-center">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-[var(--card-border)] p-8 mb-8">
              ...
              {mpesaTransactionId && (
                <>
                  <div className="h-px bg-gradient-to-r from-transparent via-[var(--brand-400)] to-transparent my-4"></div>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">M-Pesa Transaction ID</p>
                  <p className="text-md font-mono font-semibold text-green-600 dark:text-green-400">
                {mpesaTransactionId}
                  </p>
                </>
              )}
              ...
              <div className="space-y-3 text-left">
                ...
                {selectedPayment === 'mpesa' && (
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    Payment of <span className="font-semibold">KES {finalTotal.toLocaleString()}</span> has been processed via M-Pesa
                  </p>
                )}
              </div>
            </div>
              ...
              <div className="space-y-6">
                        ...
                        <div>
                          ...
                          <div className="mt-3 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                            <p className="text-xs text-blue-700 dark:text-blue-300 flex items-start gap-2">
                              ...
                              <span>You'll receive an STK push prompt on your phone. Enter your M-Pesa PIN to complete the payment.</span>
                            </p>
                          </div>
                        </div>
                      )}
                ...
                <div className="flex justify-between pt-4">
                  ...
                  <Button
                    variant="primary"
                    onClick={handlePlaceOrder}
                    disabled={isProcessing || (selectedPayment === 'mpesa' && !mpesaPhone)}
                    className="px-8 relative overflow-hidden group"
                  >
                    {isProcessing ? (
                      <span className="flex items-center gap-2">
                        {isMpesaProcessing ? (
                          <>
                            <FiSmartphone className="w-5 h-5 animate-pulse" />
                            Waiting for payment...
                          </>
                        ) : (
                          <>
                      ...
                      <>
                        <span className="relative z-10 flex items-center gap-2">
                          {selectedPayment === 'mpesa' ? 'Pay with M-Pesa' : 'Place Order'}
                          {selectedPayment === 'mpesa' ? <FiSmartphone className="w-4 h-4" /> : <FiLock className="w-4 h-4" />}
                        </span>
                        <span className="absolute inset-0 bg-gradient-to-r from-[var(--brand-600)] to-[var(--brand-700)] opacity-0 group-hover:opacity-100 transition-opacity duration-300"></span>
                      </>
                    )}
