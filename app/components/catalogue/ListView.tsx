...
import { useState } from 'react'
import Image from 'next/image'
import { FiChevronDown, FiChevronUp, FiShoppingCart, FiInfo, FiImage } from 'react-icons/fi'
import { cn } from '@/app/lib/utils'
import { catalogueItems, type CatalogueItem } from '@/app/lib/catalogue-data'
import Card from '../ui/Card'
import Button from '../ui/Button'
...
function ProductImage({ item }: { item: CatalogueItem }) {
  const [imageError, setImageError] = useState(false)
  const placeholderImage = '/images/placeholder.png'
  const imageSrc = imageError ? placeholderImage : (item.image || placeholderImage)

  return (
    <div className="relative w-12 h-12 rounded-md overflow-hidden bg-[var(--soft-gray)]">
      <Image
        src={imageSrc}
        alt={item.name}
        fill
        ...
        onError={() => {
          console.log(`Image failed for ${item.name}, switching to placeholder`)
          setImageError(true)
        }}
