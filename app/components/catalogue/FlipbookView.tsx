...
import { useState } from 'react'
import Image from 'next/image'
import { FiChevronLeft, FiChevronRight, FiZoomIn, FiZoomOut, FiDownload, FiShoppingCart, FiImage } from 'react-icons/fi'
import { cn } from '@/app/lib/utils'
import { flipbookPages, type CatalogueItem } from '@/app/lib/catalogue-data'
import Card from '../ui/Card'
import Button from '../ui/Button'
...
// Page image component with error handling
function PageImage({ 
  src, 
  alt, 
  className 
  ...
  return (
    <div className={cn('relative w-full h-full bg-[var(--soft-gray)]', className)}>
      <Image
        src={imageSrc}
        alt={alt}
        fill
        ...
        onError={() => {
          console.log(`Flipbook image failed: ${alt}, switching to placeholder`)
          setImageError(true)
        }}
      ...
      {/*{imageError && (
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          <FiImage className="w-16 h-16 text-[var(--card-border)] mb-2" />
          <span className="text-sm text-[var(--card-border)]">Page not available</span>
        </div>
      )}*/}
    </div>
  )
